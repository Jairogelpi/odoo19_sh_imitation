...
Vendored for this repository after a line-by-line audit of the upstream
`ontology` helper from ClawHub slug `ontology` version `1.0.4`.

Local contract:
- local file operations only
- no network calls
- no subprocess execution
- all file paths stay within the workspace root
...
def require_workspace_path(
    ...
    *,
    ...
    """Resolve a path relative to the current workspace root."""
...
def require_yaml():
    """Import yaml with a clearer failure mode for optional schema operations."""
    try:
        import yaml
    except ImportError as exc:
        raise SystemExit(
            "PyYAML is required for ontology schema operations in this helper."
        ) from exc
    return yaml
...
def load_graph(path: str) -> tuple[dict, list]:
    ...
    graph_path = require_workspace_path(path, label="graph path")
...
def load_schema(schema_path: str) -> dict:
    ...
    schema = {}
    schema_file = require_workspace_path(schema_path, label="schema path")
