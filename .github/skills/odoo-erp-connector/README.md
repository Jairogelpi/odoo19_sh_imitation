...
This copy is vendored inside `odoo19_sh_imitation` for inspection and selective reuse.

- In this repository it is not the preferred production path for Odoo execution.
- The local production route is `openclaw-odoo`, which uses OpenClaw requests, policy allowlists, approvals, and `openclaw.execute_request`.
- This connector uses direct XML-RPC CRUD, smart auto-create flows, and optional webhook/poller behavior, so it does not match the repo's request-first execution model as-is.
- Repo-specific vetting notes live in `docs/brain/openclaw_third_party_odoo_skills.md`.
