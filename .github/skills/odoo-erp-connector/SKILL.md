...
- This vendored copy is kept for review, comparison, and selective reuse.
- In this repository it is not the preferred production path for Odoo execution.
- The preferred operational route here is `openclaw-odoo`, which turns actions into OpenClaw requests and executes them through `openclaw.execute_request` with policy and approval checks.
- This skill's direct XML-RPC CRUD and smart auto-create workflow sit outside that request-first contract.
- See `docs/brain/openclaw_third_party_odoo_skills.md` for the repo-specific rationale.
